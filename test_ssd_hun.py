...
# 모델과 출력 위치 결정
MODEL_PATH = Path("./models/mb2-ssd-lite-Epoch-399-Loss-0.000302.pth")
OUTPUT_PATH = Path('./data/output')
OUTPUT_PATH.mkdir(parents=True, exist_ok=True) # parents = True인 경우 상위 디렉토리를 포함하여 경로의 누락 된 디렉토리를 작성, 만일 이게 False면 누락이 있으면 FileNotFoundError # exist_ok=True 경로에 폴더가 없는 경우 자동 생성

# 가져올 이미지와 csv 파일 경로
IMAGE_PATH = Path("./data/test")
DataFrame = pd.read_csv("./data/test_before.csv")

# mAP 측정을 위한 이전 값들 저장
MODEL = ['0720test', '0725test','before_model']
Before_mAP = [12.0,5.0,2.0]

# 모델에 들어간 클래스 순서에 맞게 수정
#CLASS_NAMES = ["BACKGROUND","BlackDog","CatFace","DogFace","LongDogFace"]
#CLASS_NAMES = ["BACKGROUND","DogFace","LongDogFace"]
CLASS_NAMES = ["BACKGROUND","Dogface","BlackDog","CatFace" ]
#CLASS_NAMES = ["BACKGROUND","CatFace","DogFace","LongDogFace"]

# 원하는 THRESHOLD 값 저장
THRESHOLDS = [n for n in range(50, 100, 5)] # 50 ~ 95 까지 일단 만들고
IOU_THRESHOLD = 70 # 얼만큼 예측 박스와 truth 박스의 차이가 나야 잘 나왔다고 할 지
Config_THRESHOLD = 85 # 얼만큼 이상의 confidence를 잘 찾았다고 할지
...
def make_pr_curve(recall_list : list , precision_list   : list, class_name : str, th : int,ap:float) -> None:
...
def calc_ap(recall_list :list, precision_list:list) -> float: # Precision 이랑 recall이랑 정렬해야 하지 않나? 정렬하고 , 거기에 맞게 넓이를 구해야 할것 같은데,일단 대기
    ap = 0
...
def draw_box(image, image_id:str, box:tuple, gt_box:tuple, label:str, confidence:float, THRESHOLD : int ,ClassName:str) -> None:
...
def unpredict_draw_box(image, image_id:str, box:tuple, gt_box:tuple, label:str, confidence:float, gt_label : str) -> None:
...
def compare_iou_threshold(iou:float, th:int ,accTP, accFP):
    if iou * 100 > th:
        # True Positive
...
for class_name, count in Data.items():
    ...
    for image in images:
        ...
        if len(data) == 0:
            ...
            box, label, confidence = data[0]  # 클래스에 맞는 선택된 애들
...
for class_name, c_data in image_data_dict.items():
    ...
    for data in c_data:
        ...
        accTP, accFP = compare_iou_threshold(iou, IOU_THRESHOLD, accTP, accFP)
    ...
    ap_list.append(ap)
    make_pr_curve(recall_list, precision_list,class_name,IOU_THRESHOLD,ap)
    recall_list.clear()
