...
# 모델과 출력 위치 결정
MODEL_PATH = Path("./models/mb2-ssd-lite-Epoch-299-Loss-1.3355378891740526.pth")
...
for class_name, count in Data.items():
    ...
    for image in images:
        ...
        if CLASS_NAMES[label] != gt_label: # csv에 정한 class와 맞는지 아닌지
            ...
            draw_box(orig_image, image_id, box, gt_box, label, confidence, THRESHOLD, class_name)
...
for class_name, c_data in image_data_dict.items():
    ...
    for data in c_data:
        ...
        accTP, accFP = compare_iou_threshold(iou, THRESHOLD, accTP, accFP)
    ...
    ap_list.append(ap)
    make_pr_curve(recall_list, precision_list,class_name,THRESHOLD,ap)
    recall_list.clear()
